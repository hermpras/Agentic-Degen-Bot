  ...
  parameters: {
    ...
    properties: {
      ...
      path: {
        type: "string",
        description:
          'Path file di dalam repo (contoh: "README.md" atau "src/index.ts")',
      },
      ref: {
        type: "string",
        description:
          "Branch, tag, atau commit SHA (opsional, default: default branch)",
      },
    },
  ...
  async execute(args: Record<string, any>) {
    ...
    if (!owner || !repo || !path) {
      return JSON.stringify({
        error: 'Parameter "owner", "repo", dan "path" wajib diisi.',
      });
    }
