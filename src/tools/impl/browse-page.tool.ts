...
export const browsePageTool: Tool = {
  ...
  async execute(args: Record<string, any>) {
    ...
    if (!rawUrl || typeof rawUrl !== 'string') {
      return JSON.stringify({ error: 'Parameter "url" wajib diisi dengan string URL yang valid.' });
    }
    ...
    let browser;
    try {
      console.log(`🌐 [BrowsePage] Membuka browser Playwright Chromium ke URL: "${targetUrl}"`);
      ...
      // Set timeout 15 detik untuk navigasi halaman
      await page.goto(targetUrl, {
        waitUntil: 'domcontentloaded',
        timeout: 15000,
      });

      // Tunggu hingga networkidle atau timeout 3s
      try {
      ...
      // Extract teks ter-render langsung menggunakan Playwright innerText
      ...
      // Batasi panjang teks agar tidak memenuhi token LLM (maksimal 6.000 karakter)
      const maxChars = 6000;
      ...
      console.error(`❌ [BrowsePage Error] Gagal membuka URL ${targetUrl}:`, error);
